...
class RoomService:
    ...
    def generate_room(self, height, width):
    ...
    def draw_map(self, screen, game_map):
        for x in range(len(game_map)):
            for y in range(len(game_map[x])):
                if game_map[x][y] == self.Tile.left_top_corner():
                    self.Tile.draw_tile(screen, wall, x, y)
                if game_map[x][y] == self.Tile.right_top_corner():
                    self.Tile.draw_tile(screen, wall, x, y)

                if game_map[x][y] == self.Tile.left_bottom_corner():
                    self.Tile.draw_tile(screen, wall, x, y)
                if game_map[x][y] == self.Tile.right_bottom_corner():
                    self.Tile.draw_tile(screen, wall, x, y)

                if game_map[x][y] == self.Tile.left_wall():
                    self.Tile.draw_tile(screen, wall, x, y)
                if game_map[x][y] == self.Tile.middle_wall():
                    self.Tile.draw_tile(screen, wall, x, y)
                if game_map[x][y] == self.Tile.right_wall():
                    self.Tile.draw_tile(screen, wall, x, y)

                if game_map[x][y] == self.Tile.room_floor():
